...
export default function MenuItem({ title, Image, link, submenu, setPage, page }: MenuItemProps) {
    ...
    return (
        <li className="my-1">
            <button
                className={`relative h-12 px-4 py-2 text-sm flex w-full items-center transition ${page === title ? 'bg-[#475467] text-orange-400 font-semibold' : 'hover:bg-[#475467]'} rounded-lg`}
                onClick={expandMenu}
            >
                <div className="mr-4">
                    <Image />
            ...
            {submenu && isExpanded && (
                <div className="flex flex-col">
                    {submenu.map((item) => (
                        <SubMenuItem setPage={setPage} page={page} key={item.title} title={item.title} slug={item.slug} />
                    ))}
                </div>
            )}
        </li>
