...
import { CustomerRequest, CustomersResponse, getCustomersService } from '@/services/customers.service';
                            ...
                            <tbody>
                                {customers?.data.slice(page * 10 - 10, page * 10).map((customer, k) => (
                                    <tr key={customer.user_email} className={(k + 1) % 2 === 0 ? 'bg-gray-100' : ''}>
                                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm xl:whitespace-nowrap p-4 ">{customer.user_name ? customer.user_name : '-'}</td>
                                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm xl:whitespace-nowrap p-4 ">
                                            <a href={customer.user_link ? customer.user_link : `mailto:${customer.user_email}`} target="_blank" className="text-orange-400 underline">
                                                {customer.user_email}
                                            </a>
                                        </td>
                                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm xl:whitespace-nowrap p-4 ">{customer.raf_code}</td>
                                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-sm xl:whitespace-nowrap p-4 ">{customer.num_of_orders}</td>
                                        <td
