...
export default function AdvocateRewardSettings() {
    ...
    const { data: settings, isLoading }: { data: AdvocateRewardSettingsRequest | undefined; isLoading: boolean } = useQuery('wpgens-coupon-settings', getGeneralSettings);
        ...
        if (settings) {
            ...
            if (formatted) {
                ...
                formatted['gens_raf_product_ids'] =
                    settings['gens_raf_product_ids']?.split(',').map((id) => ({ value: parseInt(id), label: wooProducts.find((p) => p.value === parseInt(id))?.label || '' })) || [];
                // Format Excluded Product IDs
                formatted['gens_raf_product_exclude_ids'] =
                    settings['gens_raf_product_exclude_ids']?.split(',').map((id) => ({ value: parseInt(id), label: wooProducts.find((p) => p.value === parseInt(id))?.label || '' })) || [];
                // Format Category IDs
                formatted['gens_raf_product_categories'] =
                    settings['gens_raf_product_categories']?.split(',').map((id) => ({ value: parseInt(id), label: wooCats.find((p) => p.value === parseInt(id))?.label || '' })) || [];
                // Format Excluded Category IDs
                formatted['gens_raf_exclude_product_categories'] =
                    settings['gens_raf_exclude_product_categories']?.split(',').map((id) => ({ value: parseInt(id), label: wooCats.find((p) => p.value === parseInt(id))?.label || '' })) || [];
        ...
        <form className="max-w-5xl px-6" onSubmit={handleSubmit(formatBeforeSubmit, (err) => console.log(err))}>
            ...
            <div className="flex flex-col mb-8">
                <div className="flex flex-col justify-between items-center bg-white p-6 rounded-xl border border-gray-200 mb-4 w-full">
                    ...
                    <ToggleField register={register} name="gens_raf_generate_for_registration" label={labels.registration.title} description={labels.registration.subtitle} />
                </div>
            </div>
            {(generateForRegistration || generateForOrder) && (
                <div className="flex flex-col mb-8">
                    ...
                    <div className="bg-white p-6 rounded-xl border border-gray-200 mb-4 w-full">
                        <WooCoupons type="advocate" subscriptionActive={!!settings?.gens_raf_subscription} errors={errors} register={register} control={control} />
                    </div>
                </div>
            )}
