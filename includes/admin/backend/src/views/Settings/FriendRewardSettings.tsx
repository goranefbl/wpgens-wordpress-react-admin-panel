...
export default function FriendRewardSettings() {
	...
	const { data: settings, isLoading }: { data: FriendRewardSettingsRequest | undefined; isLoading: boolean } = useQuery(
		'wpgens-friend-settings',
		getGeneralSettings
	);
		...
		if (settings) {
			...
			if (formatted) {
				...
				formatted['gens_raf_friend_product_ids'] =
					settings['gens_raf_friend_product_ids']
						?.split(',')
				...
				formatted['gens_raf_friend_product_exclude_ids'] =
					settings['gens_raf_friend_product_exclude_ids']
						?.split(',')
						.map((id) => ({ value: parseInt(id), label: wooProducts.find((p) => p.value === parseInt(id))?.label || '' })) || [];
				// Format Category IDs
				formatted['gens_raf_friend_product_categories'] =
					settings['gens_raf_friend_product_categories']
						?.split(',')
				...
				formatted['gens_raf_friend_exclude_product_categories'] =
					settings['gens_raf_friend_exclude_product_categories']
						?.split(',')
						.map((id) => ({ value: parseInt(id), label: wooCats.find((p) => p.value === parseInt(id))?.label || '' })) || [];
		...
		<form className='max-w-5xl px-6' onSubmit={handleSubmit(formatBeforeSubmit, (err) => console.log(err))}>
			<header className='mb-8'>
				...
				<span className='text-sm block mt-1'>
					Reward issued to user coming via referral link or referral code. You can give discount during first purchase, or AFTER he makes a purchase,
					you can send him a coupon code. Select in the options below.
				</span>
			</header>
			...
			{friendRewardAfterPurchase && (
				<div className='flex flex-col mb-8'>
					<div className=' mb-2 pr-2'>
						...
						<span className='text-sm block pr-4'>
							If enabled, this coupon will be sent after referred user makes a purchase and order is marked as completed.
						</span>
					</div>
					<div className='bg-white p-6 rounded-xl border border-gray-200 mb-4 w-full'>
						<WooCoupons
